  ...
  return (
    <main className="min-h-screen bg-[#0d0d1a] text-white">
      <nav className="border-b border-white/8 px-6 py-4 flex items-center justify-between">
        ...
        <Link href="/" className="text-sm text-white/50 hover:text-white/80 transition-colors flex items-center gap-1">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <div>
              <h1 className="text-2xl font-bold leading-snug mb-2">{video.title}</h1>
              <div className="flex items-center gap-3 text-sm text-white/50">
                ...
                <a
                  ...
                  className="flex items-center gap-1 text-sky-400/80 hover:text-sky-300 transition-colors"
                >
            ...
            <div className="bg-white/4 border border-white/8 rounded-2xl p-5">
              <p className="text-xs text-sky-400/70 font-bold tracking-widest mb-2 uppercase">Why this passed</p>
              <p className="text-white/70 leading-relaxed text-sm">{video.scoreRationale}</p>
            </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {video.tags.map((tag: string) => (
                  <Link
                    key={tag}
                    href={`/?tag=${tag}`}
                    className="text-xs text-sky-400/80 bg-sky-500/10 border border-sky-500/20 rounded-full px-3 py-1 hover:bg-sky-500/20 transition-colors"
                  >
                    {tag}
                  </Link>
                ))}
          ...
          {/* Right: scorecard */}
          <div className="bg-white/4 border border-white/8 rounded-2xl p-5 flex flex-col gap-5 h-fit">
            <div className="flex items-baseline justify-between">
              <p className="text-xs text-white/40 font-bold tracking-widest uppercase">Signal Score</p>
              <p className="text-2xl font-bold text-sky-400">{video.scores?.overall?.toFixed(1)}</p>
            </div>
